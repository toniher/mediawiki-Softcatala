...
function render_email_field($arg, $value, $ispost, $input, &$parser, $extra) {
    ...
    case 'form':    # Form wrapper
        ...
        if ( $ispost ) {
            ...
            if ( $wgTitle->mNamespace != NS_REBOST )
            //            if ( !($wgTitle->isProtected ('edit')) )
		    return ( "<em>". EMAILFORM_NAME . " només funciona al Rebost.</em>" );
            $parser->disableCache();     #### Important: Disable Cache ####
            $formurl = $wgTitle->getFullURL();
	    if (preg_match('/\:\/\/www/', $formurl) == 0) {
	    	$formurl = preg_replace('/http\:\/\//', 'http://www.', $formurl);
	    }
	    return (
...
#    }
    ...
    if ( $name ) {
        if ( $from )    $from = $from;
        else            $from = 'avis_rebost@softcatala.org';
    }
    ...
    $subject = "[Rebost] ". $settings['prefix'] . " - ".  $name;       #This reduces the subject line to just the prefix and the name
...
"From: " . "avis_rebost@softcatala.org" . "\n".
"Reply-To: " . $from ."\n";
$headers .= "X-Sender: <" . $from . ">\n";
$headers .= "X-Mailer: PHP\n";
$headers .= "X-Priority: 6\n";

$message = $message."\nhttp://www.softcatala.org/wiki/Rebost:".$settings['prefix']."\n";
...
#    }
    ...
    $email_address = "rebost@llistes.softcatala.org";
		
    return( array( 'to'=>trim($email_address), 'prefix'=>trim(preg_replace('/\s/', '_', $pagetitle) ) ) );
